...
//    }
    ...
    id object = [results_ objectAtIndex:indexPath.row];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    if ([[self dataSource] respondsToSelector:@selector(sourceForAutoCompletionTextField:withParam:)]){
        // get the source
        NSString *param = [textField.text stringByReplacingOccurrencesOfString:@" " withString:@"+"];
...
- (void)loadSearchTableWithResults:(NSArray *)searchResults
{
    ...
    int totalCompletionCount = [results_ count];
    [[self tableView] reloadData];
